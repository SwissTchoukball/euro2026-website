...
import { generateStandingsTeams, sortStandings } from "./standings";
...
describe("generateStandingsTeams", () => {
  ...
  it("counts a win correctly (3 pts winner, 1 pt loser)", () => {
  ...
  it("counts a win for team B correctly", () => {
  ...
  it("counts a draw correctly (2 pts each)", () => {
  ...
  it("handles forfeit (30-0): loser gets 0 points", () => {
  ...
  it("handles forfeit when team B forfeits (0-30): team A gets 0 points", () => {
...
describe("sortStandings", () => {
  ...
  it("uses head-to-head to break a two-way tie", () => {
  ...
  it("breaks ties by overall goal difference when head-to-head is equal", () => {
    ...
    const games = [
      makeEndedGame(teamA, 60, teamC, 30), // A: +30
      makeEndedGame(teamB, 50, teamD, 30), // B: +20
      makeEndedGame(teamA, 40, teamD, 30), // A: +10 (total diff: +40)
      makeEndedGame(teamB, 50, teamC, 30), // B: +20 (total diff: +40)
    ];
    const standings = generateStandingsTeams(games);
    const sorted = sortStandings(standings, games);

    // A and B both have 6 pts, no head-to-head, same diff (+40)
    // A: for=100, B: for=100 → same, fallback to name
    expect(sorted[0]!.team.id).toBe("1"); // Alpha before Bravo
    expect(sorted[1]!.team.id).toBe("2");
  });

  it("sorts by points-for when goal difference is equal", () => {
